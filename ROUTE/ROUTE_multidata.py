...
class Config:
    ...
    acl_mode: str = "all_pairs"

    # 下列在 all_pairs 下不会用到，但保留也没事
...
def load_acl_task(cfg: Config, base_dir: str, src_domain: str, tgt_domain: str) -> Tuple[DataBundle, Dict[str, Any]]:
    ...
    leakage_report(
        src_splits={
            "train": data.src_train_texts,
            "val": data.src_val_texts,
            "test": data.src_test_texts,
        },
        tgt_splits={
            "train": data.tgt_train_texts,
            "val": data.tgt_val_texts,
            "test": data.tgt_test_texts,
        },
    )
...
def run_seed(seed: int, cfg: Config, data: DataBundle, seed_dir: str, device: torch.device) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    for cand in candidates:
        ...
        try:
            ...
            Xt = encode_cls_embeddings(
                m, tokenizer, data.tgt_val_texts, cfg, device,
                max_samples=cfg.proto_max_tgt_val_samples
            )
            n = int(Xt.shape[0])
            ...
            if n == 0:
                ...
                # 用同样前 n 条的概率，保证长度一致
                probs_sub = tgt_val_probs[:n] if tgt_val_probs.shape[0] >= n else tgt_val_probs
                ypv = probs_sub.argmax(axis=1).astype(np.int64)
