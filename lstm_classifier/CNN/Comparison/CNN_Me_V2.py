...
covid_fake_dfv1 = pd.read_csv('saveddata/fake_news.csv')  # Fake medical news
covid_real_dfv1 = pd.read_csv('saveddata/true_News.csv')  # Real medical news
...
covid_fake_textsv1 = filtered_df['Text']

# Medical: real
filtered_df = covid_real_df[(covid_real_df['Text'].str.len() >= 40)]
covid_real_textsv1 = filtered_df['Text']

# Medical: fake
filtered_df = covid_fake_dfv1[(covid_fake_dfv1['text'].str.len() >= 40)]
covid_fake_texts = filtered_df['text']

# Medical: real
filtered_df = covid_real_dfv1[(covid_real_dfv1['text'].str.len() >= 40)]
...
medical_textsv1 = pd.concat([covid_fake_textsv1, covid_real_textsv1])
medical_labelsv1 = np.concatenate([
    np.zeros(len(covid_fake_textsv1)),
    np.ones(len(covid_real_textsv1))
])
...
medical_sequencesv1 = tokenizer.texts_to_sequences(medical_textsv1)
...
medical_datav1 = pad_sequences(medical_sequencesv1, maxlen=maxlen)

policy_labels = np.array(policy_labels)
medical_labels = np.array(medical_labels)
medical_labelsv1 = np.array(medical_labelsv1)
# ======================
# 5. 拆分训练集和验证集
# ======================
...
X_train_m_fullv1, X_val_mv1, y_train_m_fullv1, y_val_mv1 = train_test_split(
    medical_datav1, medical_labelsv1, test_size=0.2, random_state=42
)
...
for frac in fractions:
    ...
    y_pred_prob = model_transfer.predict(X_val_mv1)
    y_pred = (y_pred_prob > 0.5).astype("int32")
    acc_m = accuracy_score(y_val_mv1, y_pred)
    medical_acc_transfer.append(acc_m)
    evaluate_and_print(model_transfer, X_val_mv1, y_val_mv1, "Medical Val (Transfer)")
