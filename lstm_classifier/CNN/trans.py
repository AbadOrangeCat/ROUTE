...
from tensorflow.keras.layers import (
    Embedding, Dense, Dropout, Input, MultiHeadAttention,
...
X_train2, X_test2, y_train2, y_test2 = train_test_split(
    df2['cleaned_text'], df2['label'], test_size=0.2, random_state=42
)
...
embedding_layer = Embedding(
    input_dim=MAX_NUM_WORDS, output_dim=EMBEDDING_DIM, input_length=MAX_SEQUENCE_LENGTH
)
...
position_embedding = Embedding(
    input_dim=MAX_SEQUENCE_LENGTH, output_dim=EMBEDDING_DIM
)(positions)
x = x + position_embedding

# 添加多个Transformer编码器层
for _ in range(2):  # 可以根据需要调整层数
