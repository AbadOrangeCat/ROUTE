...
"""
DUPL V7 (non-LLM UDA) - V6 + R-Drop + Warmup/Cosine + MC-dropout uncertainty filter

Source: PubHealth (claim+main_text, label: 0=true, 1=false)
Target: COVID (Text only, unlabeled for training; labels ONLY used for evaluation at the end)

Key upgrades in V7:
1) R-Drop (two stochastic forward passes) for source supervision and optional pseudo batches
2) Warmup + Cosine LR scheduler for stable optimization
3) Teacher MC-dropout uncertainty estimation for pseudo-label filtering (entropy + variance)
4) Distribution Alignment used in training + inference; match-prior threshold at inference (optional)
5) Optional pretrained word embeddings (GloVe/FastText .txt), freeze or finetune

Note:
- Training NEVER uses target labels.
- Evaluation uses target labels only for reporting.
"""
...
def symmetric_kl(p_logits: torch.Tensor, q_logits: torch.Tensor) -> torch.Tensor:
    """
    0.5 * ( KL(p||q) + KL(q||p) ) where p,q are softmax distributions.
    """
    ...
    kl_pq = F.kl_div(p_logp, q, reduction="batchmean")
    kl_qp = F.kl_div(q_logp, p, reduction="batchmean")
    return 0.5 * (kl_pq + kl_qp)


def ce_label_smoothing(logits: torch.Tensor, y: torch.Tensor, smoothing: float) -> torch.Tensor:
    if smoothing <= 0:
    ...
    loss = -(true_dist * logp).sum(dim=1).mean()
    return loss
...
def set_optimizer_lr(optim: torch.optim.Optimizer, base_lr: float, mult: float) -> None:
...
@torch.no_grad()
def teacher_predict_with_mc_dropout(
    ...
    Returns:
      mean_probs: (B,C)
      var_scalar: (B,)  mean variance across classes
    """
    if mc_passes <= 1:
        _f, logits = teacher(input_ids, mask)
        probs = torch.softmax(logits / max(1e-6, temperature), dim=-1)
        var = torch.zeros(probs.size(0), device=probs.device)
        return probs, var

    # enable dropout by switching to train mode temporarily (no grad)
    teacher.train()
    ...
    stack = torch.stack(probs_list, dim=0)  # (K,B,C)
    mean_probs = stack.mean(dim=0)
    var = stack.var(dim=0).mean(dim=1)  # (B,)
    teacher.eval()
...
def build_pseudo_pool(
    ...
    feats_all, probs_all, var_all, idx_all = [], [], [], []
    ...
    for weak_ids, weak_mask, _strong_ids, _strong_mask, idxs in tqdm(target_loader, desc="Teacher forward (target)", leave=False):
        ...
        probs, var = teacher_predict_with_mc_dropout(
            teacher=teacher,
            input_ids=weak_ids,
            mask=weak_mask,
        ...
        feats, _logits_unused = teacher(weak_ids, weak_mask)  # features once (eval mode)
        feats_all.append(feats.cpu().numpy())
        probs_all.append(probs.cpu().numpy())
        var_all.append(var.cpu().numpy())
        idx_all.append(idxs.numpy())

    feats = np.concatenate(feats_all, axis=0)
    probs = np.concatenate(probs_all, axis=0)
    var = np.concatenate(var_all, axis=0)
    idxs = np.concatenate(idx_all, axis=0)

    order = np.argsort(idxs)
    feats, probs, var, idxs = feats[order], probs[order], var[order], idxs[order]

    # update EMA of mean probs (before DA)
    ...
    # normalize var to [0,1] roughly (robust)
    v = var.astype(np.float32)
    v_med = np.median(v)
    v_mad = np.median(np.abs(v - v_med)) + 1e-6
    v_norm = np.clip((v - v_med) / (5.0 * v_mad), -1.0, 3.0)  # robust z-ish
    v_norm = np.clip((v_norm + 1.0) / 4.0, 0.0, 1.0)

    # score for selection
    score = conf * (1.0 - ent) * (1.0 - v_norm)
    ...
    def pick_for_class(y: int) -> List[int]:
        ...
        conf_thr, ent_thr, var_thr = float(args.tau_conf), float(args.tau_ent), float(args.tau_var)

        for _step in range(int(args.relax_steps) + 1):
            cand = np.where((pred == y) & (conf >= conf_thr) & (ent <= ent_thr) & (v_norm <= var_thr))[0]
            if cand.size > 0:
                # per-cluster top-m
                for c in range(k):
                    if need <= 0:
            ...
            var_thr = min(1.0, var_thr + float(args.var_step))
    ...
    # weights: scaled score + class-balance
    ...
    if args.print_pseudo_stats:
        ...
        print(
            ...
            f"dist(source0/1)={dist} p_ema=[{new_p_ema[0]:.2f},{new_p_ema[1]:.2f}]"
        )
...
def train_v7(
    ...
    # target prior (you can set uniform or source)
    ...
    steps_pretrain = args.pretrain_epochs * max(1, len(src_train_loader))
    steps_per_uda_epoch = max(1, max(len(src_train_loader), len(tgt_loader)))
    steps_uda = args.uda_epochs * steps_per_uda_epoch
    total_steps = steps_pretrain + steps_uda
    ...
    for ep in range(1, args.pretrain_epochs + 1):
        ...
        for ids, mask, y in pbar:
            ...
            _f1, logits1 = student(ids, mask)
            if args.rdrop_alpha > 0:
                _f2, logits2 = student(ids, mask)
                loss_ce = 0.5 * (
                    ce_label_smoothing(logits1, y, args.label_smoothing)
                    + ce_label_smoothing(logits2, y, args.label_smoothing)
                )
                loss_rdrop = symmetric_kl(logits1, logits2)
                loss = loss_ce + args.rdrop_alpha * loss_rdrop
            else:
                loss = ce_label_smoothing(logits1, y, args.label_smoothing)

            # lr schedule
            mult = lr_multiplier(global_step, total_steps, warmup_steps, args.min_lr_ratio)
            set_optimizer_lr(optimizer, args.lr, mult)
            ...
            pbar.set_postfix(sup=float(loss.detach().cpu()), lr=float(optimizer.param_groups[0]["lr"]))
    ...
    for uda_ep in range(1, args.uda_epochs + 1):
        ...
        pbar = tqdm(range(steps_per_uda_epoch), desc=f"UDA {uda_ep}/{args.uda_epochs}", leave=True)
        for _ in pbar:
            try:
            ...
            # ---------- source supervised (with optional R-Drop) ----------
            src_feats, src_logits1 = student(src_ids, src_mask)
            if args.rdrop_alpha > 0:
                _sf2, src_logits2 = student(src_ids, src_mask)
                loss_sup = 0.5 * (
                    ce_label_smoothing(src_logits1, src_y, args.label_smoothing)
                    + ce_label_smoothing(src_logits2, src_y, args.label_smoothing)
                ) + args.rdrop_alpha * symmetric_kl(src_logits1, src_logits2)
            else:
                loss_sup = ce_label_smoothing(src_logits1, src_y, args.label_smoothing)

            # ---------- target: student strong ----------
            tgt_feats, tgt_logits_s = student(tgt_s_ids, tgt_s_mask)
            tgt_probs_s = torch.softmax(tgt_logits_s, dim=-1)

            # ---------- teacher weak (with DA) ----------
            with torch.no_grad():
                probs_w, _var_w = teacher_predict_with_mc_dropout(
                    teacher=teacher,
                    input_ids=tgt_w_ids,
                    mask=tgt_w_mask,
                    mc_passes=1,  # in-step consistency uses single pass for speed
                    temperature=args.teacher_temp,
                )
                if args.use_da:
                    probs_w = distribution_align_torch(probs_w, p_ema_t, pi_target_t)

            # consistency loss
            logp_s = torch.log_softmax(tgt_logits_s, dim=-1)
            kl = F.kl_div(logp_s, probs_w, reduction="none").sum(dim=1)

            ent_w = -(probs_w * torch.log(probs_w.clamp_min(1e-12))).sum(dim=1) / math.log(2.0)
            ...
            # ---------- pseudo-label loss (optional R-Drop on pseudo) ----------
            ...
            if pseudo_iter is not None and alpha_pl_eff > 0:
                ...
                _pf, pl_logits1 = student(pl_ids, pl_mask)
                logp1 = torch.log_softmax(pl_logits1, dim=-1)
                kl1 = F.kl_div(logp1, pl_soft, reduction="none").sum(dim=1)
                loss_pl_main = (kl1 * pl_w).mean()

                if args.rdrop_alpha_pl > 0:
                    _pf2, pl_logits2 = student(pl_ids, pl_mask)
                    loss_pl_rdrop = symmetric_kl(pl_logits1, pl_logits2)
                    loss_pl = loss_pl_main + args.rdrop_alpha_pl * loss_pl_rdrop
                else:
                    loss_pl = loss_pl_main
...
def infer_predict_labels(
    probs_source: np.ndarray,  # in source label space (0=true,1=false)
    infer_match_prior: bool,
    pi_target: np.ndarray,      # in source label space
) -> np.ndarray:
...
def main() -> None:
    ...
    parser.add_argument("--out_csv", type=str, default=str(base_dir / "covid_predictions_dupl_v7.csv"))
    ...
    parser.add_argument("--max_emb_lines", type=int, default=0)
    ...
    # R-Drop + smoothing
    parser.add_argument("--rdrop_alpha", type=float, default=0.5, help="R-Drop weight for source CE")
    parser.add_argument("--rdrop_alpha_pl", type=float, default=0.1, help="R-Drop weight on pseudo batches (small)")
    ...
    parser.add_argument("--tau_conf", type=float, default=0.96)
    parser.add_argument("--tau_ent", type=float, default=0.55)
    parser.add_argument("--tau_var", type=float, default=0.55, help="max normalized variance for pseudo acceptance")
    parser.add_argument("--var_step", type=float, default=0.05)
    ...
    # Teacher temperature + MC-dropout
    parser.add_argument("--teacher_temp", type=float, default=1.0)
    parser.add_argument("--mc_passes", type=int, default=2, help="MC-dropout passes for pseudo pool (2~4 is ok)")
    ...
    parser.add_argument("--infer_match_prior", action="store_true")
    ...
    teacher, vocab, pi_target, p_ema_last = train_v7(
    ...
    pred_covid = infer_predict_labels(probs_adj, infer_match_prior=args.infer_match_prior, pi_target=pi_target)
    ...
    out_df = pd.DataFrame(
        {
            ...
            "pred_covid_label(1=true,0=false)": pred_covid,
    ...
    print(f"[Info] Predicted COVID label distribution (0/1): {pd.Series(pred_covid).value_counts().to_dict()}")

    if eval_available and eval_y is not None:
        acc = accuracy_score(eval_y, pred_covid)
        cm = confusion_matrix(eval_y, pred_covid, labels=[0, 1])
        print(f"\n[Eval] COVID Accuracy (labels used ONLY for evaluation): {acc:.6f}")
        print("[Eval] Confusion matrix (rows=true [0,1], cols=pred [0,1]):")
        print(cm)
        print("\n[Eval] Classification report:")
        print(classification_report(eval_y, pred_covid, digits=4))
