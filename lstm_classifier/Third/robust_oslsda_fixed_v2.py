    ...
    for ep in range(1, epochs + 1):
        for x_w, x_s, _ in tqdm(stream_loader_train, desc=f"[ours] epoch {ep}/{epochs}"):
            ...
            if (step % update_pi_every) == 0:
                # NOTE: `estimate_pi_target` limits the amount of data via `max_batches`.
                # A previous revision mistakenly passed `iters=` (non-existent kwarg),
                # which would crash with: "unexpected keyword argument 'iters'".
